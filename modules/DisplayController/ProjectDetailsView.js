...
import { addTodoToProject, removeTodoFromProject, toggleTodoCompletion } from "../Todo";
...
function renderProjectView(projectId) {
    ...
    if (project) {
        ...
        document.getElementById('add-todo-btn').addEventListener('click', () => handleAddTodoClick(projectId));

        document.querySelectorAll('.todo-item input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                const todoId = parseInt(e.target.closest('.todo-item').getAttribute('data-todo-id'), 10);
